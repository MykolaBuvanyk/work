import React, { useState } from 'react';
import './Contacts.scss';
import { Link } from 'react-router-dom';
import { $host } from '../../http';
import { useTranslation } from 'react-i18next';

const Contacts = () => {
  const {t}=useTranslation()
  const [errors, setErrors] = useState({});
  const phoneNumber = '+4915776625125';
  const displayPhoneNumber = '+49 157 766 25 125';

  const validateContactForm = (data) => {
    const nextErrors = {};
    const email = String(data.email || '').trim();
    const question = String(data.question || '').trim();

    if (!email) {
      nextErrors.email = 'contacts.description_99';
    } else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
      nextErrors.email = 'contacts.description_100';
    }

    if (!question) {
      nextErrors.question = 'contacts.description_101';
    }

    return nextErrors;
  };

  const clearFieldError = (fieldName) => {
    setErrors((currentErrors) => {
      if (!currentErrors[fieldName]) return currentErrors;
      const { [fieldName]: _removed, ...rest } = currentErrors;
      return rest;
    });
  };

  const sendWithContact = async (e) => {
    e.preventDefault();

    const form = e.target;
    const formData = new FormData(form);

    const data = {
      name: formData.get('name'),
      email: formData.get('email'),
      question: formData.get('question'),
    };

    const nextErrors = validateContactForm(data);
    setErrors(nextErrors);

    if (Object.keys(nextErrors).length > 0) return;

    try {
      await $host.post('email/contact', data);

      alert(t('contacts.description_102'));

      form.reset();
      setErrors({});
    } catch (err) {
      alert(t('contacts.description_103'));
    }
  };

  return (
    <div className='contacts-container'>
      <h1>{t("contacts.description_90")}</h1>

      <div className="contact-grid">
        <div className="info-column">
          <section>
            <div className="section-title2">{t("contacts.description_91")}</div>

            <a
              className="whatsapp-link"
              href={`https://wa.me/${phoneNumber.replace('+', '')}`}
              target="_blank"
              rel="noopener noreferrer"
              aria-label={`WhatsApp ${displayPhoneNumber}`}
            >
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                <path fillRule="evenodd" clipRule="evenodd" d="M19.4405 4.55161C18.4697 3.58039 17.3165 2.81077 16.0471 2.28706C14.7777 1.76334 13.4172 1.49587 12.0441 1.50005C6.28125 1.50005 1.59 6.16786 1.58766 11.9063C1.58511 13.7335 2.06676 15.5288 2.98359 17.1094L1.5 22.5L7.04297 21.053C8.57729 21.8842 10.2948 22.3192 12.0398 22.3186H12.0441C17.8064 22.3186 22.4972 17.6504 22.5 11.9124C22.5035 10.5439 22.2348 9.18839 21.7095 7.92471C21.1843 6.66103 20.413 5.51442 19.4405 4.55161ZM12.0441 20.5622H12.0403C10.4847 20.5627 8.95739 20.1464 7.61719 19.3566L7.29984 19.1691L4.01062 20.0279L4.88859 16.8361L4.68187 16.508C3.81206 15.1309 3.35121 13.5351 3.35297 11.9063C3.35297 7.1377 7.25344 3.25786 12.0473 3.25786C14.347 3.25375 16.5542 4.16327 18.1833 5.78636C19.8125 7.40944 20.7302 9.61318 20.7347 11.9129C20.7328 16.6819 16.8342 20.5622 12.0441 20.5622ZM16.8108 14.0846C16.5497 13.9543 15.2639 13.3252 15.0258 13.2385C14.7877 13.1518 14.6119 13.1082 14.438 13.3688C14.2641 13.6294 13.763 14.2126 13.6106 14.3883C13.4583 14.5641 13.3059 14.5833 13.0448 14.453C12.7837 14.3227 11.9414 14.0485 10.9434 13.1625C10.1667 12.473 9.64266 11.6218 9.49031 11.3616C9.33797 11.1015 9.4739 10.9604 9.60469 10.831C9.72234 10.7143 9.86578 10.5272 9.99656 10.3754C10.1273 10.2235 10.1709 10.1147 10.2577 9.9413C10.3444 9.76786 10.3012 9.61599 10.2361 9.48614C10.1709 9.3563 9.64828 8.07614 9.43078 7.55536C9.21844 7.04817 9.00328 7.11708 8.84297 7.10911C8.69062 7.10161 8.51484 7.09974 8.34187 7.09974C8.20965 7.10318 8.07955 7.13384 7.9597 7.18981C7.83985 7.24579 7.73283 7.32586 7.64531 7.42505C7.40578 7.68567 6.73078 8.31567 6.73078 9.59442C6.73078 10.8732 7.66828 12.1107 7.79766 12.2841C7.92703 12.4575 9.63984 15.084 12.2606 16.2104C12.7473 16.4188 13.2445 16.6015 13.7503 16.7579C14.3761 16.9557 14.9456 16.928 15.3956 16.861C15.8977 16.7865 16.9425 16.2319 17.1595 15.6244C17.3766 15.0169 17.377 14.4966 17.3119 14.3883C17.2467 14.2801 17.0723 14.2144 16.8108 14.0846Z" fill="#1E293B"/>
              </svg>
              <span>Whats App</span>
            </a>

            <a className='contact-text' href={`tel:${phoneNumber}`}>
              {displayPhoneNumber}
            </a>
          </section>

          <section>
            <div className="section-title2">E-mail</div>
            <Link to={'mailto:info@sign-xpert.com'} className="email-link">
              info@sign-xpert.com
            </Link>
          </section>

          <section>
            <div className="section-title2">{t("contacts.description_92")}</div>
            <div className="contact-text address-box">
              Baumwiesen 2<br />
              Haigerloch<br />
              72401 Germany
            </div>
          </section>

          <section>
            <div className="section-title2">{t("contacts.description_93")}</div>
            <div className="contact-text">{t("contacts.description_94")}</div>
          </section>
        </div>

        <div className="form-column">
          <div className="form-title2">
           {t("contacts.description_95")}
          </div>

          <form onSubmit={sendWithContact} noValidate>
            <div className="input-group">
              <label htmlFor="name">{t("contacts.description_96")}</label>
              <input type="text" id="name" name="name" />
            </div>

            <div className="input-group">
              <label htmlFor="email">*E-mail</label>
              <input
                type="email"
                id="email"
                name="email"
                aria-invalid={Boolean(errors.email)}
                aria-describedby={errors.email ? 'email-error' : undefined}
                className={errors.email ? 'input-error' : ''}
                onChange={() => clearFieldError('email')}
              />
              {errors.email && <div id="email-error" className="field-error">{t(errors.email)}</div>}
            </div>

            <div className="input-group">
              <label htmlFor="question">{t("contacts.description_97")}</label>
              <textarea
                id="question"
                name="question"
                rows={5}
                aria-invalid={Boolean(errors.question)}
                aria-describedby={errors.question ? 'question-error' : undefined}
                className={errors.question ? 'input-error' : ''}
                onChange={() => clearFieldError('question')}
              />
              {errors.question && <div id="question-error" className="field-error">{t(errors.question)}</div>}
            </div>

            <button type="submit" className="send-btn">
              {t("contacts.description_98")}
            </button>
          </form>
        </div>
      </div>
    </div>
  );
};

export default Contacts;
