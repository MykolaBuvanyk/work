...
const IconMenu = ({ isOpen, onClose }) => {
  ...
  const categories = [
    'Animals', 'Arrows', 'Basic_Shapes', 'building', 'Celebrations', 'Chapter_Calendar',
    'Children', 'Electronics', 'Emoji', 'Fire_Safety', 'First_Aid', 'Food', 'frame',
    'frames', 'Horoscope', 'Mandatory', 'Maritime', 'Nature', 'People', 'Prohibition',
    'Tools', 'Transport', 'Warning', 'Work_and_School',
  ];
  ...
  const potentialIcons = {
    ...
    frame: ['frame_1.svg'],
    frames: Array.from({ length: 49 }, (_, i) => `frames_${i + 1}.svg`).filter(n => !['frames_10', 'frames_11', 'frames_21', 'frames_25'].some(m => n.includes(m))),
  ...
  const createImageFromSVG = async (svgText, iconName) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      img.onload = () => {
        ...
        ctx.drawImage(img, (80 - img.width * scale) / 2, (80 - img.height * scale) / 2, img.width * scale, img.height * scale);
        URL.revokeObjectURL(url);
        fabric.Image.fromURL(canvas2D.toDataURL(), (imgObj) => {
          imgObj.set({ left: canvas.getWidth() / 2, top: canvas.getHeight() / 2, originX: 'center', originY: 'center' });
          resolve(imgObj);
        }, { crossOrigin: 'anonymous' });
      };
      img.onerror = () => { URL.revokeObjectURL(url); reject(new Error('Не вдалося завантажити зображення')); };
      img.src = url;
    });
  };
